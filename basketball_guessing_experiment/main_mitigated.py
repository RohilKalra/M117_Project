...
class PlayerGuessingGame:
    ...
    def eliminate_incorrect_fact(self):
        prompt = f"Imagine you are playing four truths and one lie, and the following sentences refer to some active or retired NBA player. [{','.join(self.facts)}]. Which sentence is the lie? Respond ONLY with the index of the sentence that is incorrect (index from 1 to 5)."
        response = self.llama_llm.complete(prompt)
    ...
    def play_game(self):
        ...
        for i in range(5):
            ...
            guessed_players.append(guessed_player)
        ...
        self.eliminate_incorrect_fact()
        final_guessed_player = self.guess_player()
        guessed_players.append(final_guessed_player)

        prompt = f"Generate a statement for {final_guessed_player} in the following format: Paul Pierce, Retired, Small Forward, 6'7\", 1-time MVP, 1-time Champion, 20.0 PPG, 6.8 RPG, 3.5 APG"
        response = self.llama_llm.complete(prompt)
        guessed_statistic = response.text.strip()
        ...
        return {
            ...
            'guessed_player': final_guessed_player,
